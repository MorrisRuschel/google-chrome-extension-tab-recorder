...
(function () {
  ...
  let recordingIsAudioOnly = false;
  ...
  function startRecording(streamId, audioOnly) {
    recordingIsAudioOnly = audioOnly === true;
    ...
    navigator.mediaDevices.getUserMedia(constraints)
      .then((s) => {
        ...
        const videoTrack = recordingIsAudioOnly ? null : stream.getVideoTracks()[0];
        const audioTrack = dest.stream.getAudioTracks()[0];
        const combinedStream = recordingIsAudioOnly
          ? new MediaStream([audioTrack].filter(Boolean))
          : new MediaStream([videoTrack, audioTrack].filter(Boolean));
        ...
        if (recordingIsAudioOnly) {
          mimeType = MediaRecorder.isTypeSupported('audio/webm;codecs=opus')
            ...
            : 'audio/webm';
          recorderOpts = { mimeType, audioBitsPerSecond: 128000 };
        } else {
          mimeType = MediaRecorder.isTypeSupported('video/webm;codecs=vp9,opus')
            ? 'video/webm;codecs=vp9,opus'
            : 'video/webm';
          recorderOpts = { mimeType, videoBitsPerSecond: 2500000 };
        }
        ...
        mediaRecorder.onstop = () => {
          ...
          const blobType = recordingIsAudioOnly ? 'audio/webm' : 'video/webm';
  ...
  function handleCommand(msg) {
    if (msg.action === 'START_RECORDING' && msg.streamId) {
      startRecording(msg.streamId, msg.audioOnly === true);
      return;
    }
    if (msg.action === 'STOP') {
  ...
  function updateUI(s) {
    ...
    statusText.textContent =
      s.state === 'recording' ? recordingLabel : s.state === 'paused' ? pausedLabel : 'Aguardando…';
